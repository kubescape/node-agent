...
func main() {
	...
	if cfg.EnableRuntimeDetection {
		...
		// gateway) at projection time. Services and EndpointSlices are watched
		// cluster-wide (a profile may reference any namespace's Service); the
		// Node informer is field-selected to this agent's own node — the "host"
		// entity is local, and a cluster-wide Node watch on every DaemonSet pod
		// is O(nodes^2) traffic for no benefit.
		...
		// Advance the lister generation on any Service/EndpointSlice/Node change,
		// so the reconciler re-projects serviceRef/entity profiles when the
		// cluster view moves (endpoint churn, or caches that fill after startup).
		// Per-event cost is a single atomic increment; the re-projection itself
		// is coalesced onto the reconcile tick, and only serviceRef-using
		...
		_, _ = svcInformers.Core().V1().Services().Informer().AddEventHandler(bump)
		_, _ = svcInformers.Discovery().V1().EndpointSlices().Informer().AddEventHandler(bump)
		_, _ = nodeInformers.Core().V1().Nodes().Informer().AddEventHandler(bump)
		// Start the informers and hand the lister over WITHOUT blocking on cache
		// sync: node-agent's core startup (container watcher, profiling) must not
		// wait on these, and a bounded wait here previously delayed learning
		// enough to trip the tight completion budget of Test_22. The caches fill
		// in the background; serviceRef/entity neighbors resolve on the next
		// reconcile once populated. nil-until-set is a no-op in projection.
