...
func TestGetDestinationPodLabels(t *testing.T) {
	tests := []struct {
		name     string
		input    NetworkEvent
		expected map[string]string
		...
		{
			name: "multiple pod labels",
			input: NetworkEvent{
				Destination: Destination{
					PodLabels: "app=nginx,version=v1.0,env=prod",
				},
			},
			expected: map[string]string{"app": "nginx", "version": "v1.0", "env": "prod"},
		},
		{
			name: "invalid label format - no equals sign",
			input: NetworkEvent{
				Destination: Destination{
					PodLabels: "app-nginx,version=v1.0",
				},
			},
			expected: map[string]string{"version": "v1.0"},
		},
		{
			name: "invalid label format - multiple equals signs",
			input: NetworkEvent{
				Destination: Destination{
					PodLabels: "app=nginx=test,version=v1.0",
				},
			},
			expected: map[string]string{"version": "v1.0"},
		},
		{
			...
			input: NetworkEvent{
				Destination: Destination{
					PodLabels: "app.kubernetes.io/name=nginx,app.kubernetes.io/version=1.0",
				},
			},
			expected: map[string]string{"app.kubernetes.io/name": "nginx", "app.kubernetes.io/version": "1.0"},
		},
		{
			name: "empty label values",
			input: NetworkEvent{
				Destination: Destination{
					PodLabels: "app=,version=v1.0",
				},
			},
			expected: map[string]string{"app": "", "version": "v1.0"},
		},
	}

	for _, tc := range tests {
		t.Run(fmt.Sprintf("Input: %s", tc.name), func(t *testing.T) {
			result := tc.input.GetDestinationPodLabels()
			assert.Equal(t, tc.expected, result)
		})
	}
}
