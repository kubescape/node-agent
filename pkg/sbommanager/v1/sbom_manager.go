...
func (s *SbomManager) ContainerCallback(notif containercollection.PubSubEvent) {
	...
	imageID := fmt.Sprintf("%s@%s", notif.Container.Runtime.ContainerImageName, notif.Container.Runtime.ContainerImageDigest)
	...
	case err != nil:
		logger.L().Ctx(s.ctx).Warning("SbomManager - failed to create empty SBOM before processing",
			helpers.Error(err),
			helpers.String("namespace", notif.Container.K8s.Namespace),
			helpers.String("pod", notif.Container.K8s.PodName),
			helpers.String("container", notif.Container.K8s.ContainerName),
	...
	// get image layers
	layers, err := s.getImageLayers(notif.Container.Runtime.ContainerImageName)
	if err != nil {
	...
	src := NewSource(notif.Container.Runtime.ContainerImageName, notif.Container.Runtime.ContainerImageDigest, layers, mounts)
	// create the SBOM
