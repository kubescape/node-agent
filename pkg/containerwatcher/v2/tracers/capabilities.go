...
func (ct *CapabilitiesTracer) Start(ctx context.Context) error {
	ct.gadgetCtx = gadgetcontext.New(
		...
		"ghcr.io/inspektor-gadget/gadget/trace_capabilities:v0.44.1",
		...
		simple.OnInit(func(gadgetCtx operators.GadgetContext) error {
			for _, d := range gadgetCtx.GetDataSources() {
				err := d.Subscribe(func(source datasource.DataSource, data datasource.Data) error {
					ct.callback(&utils.EnrichEvent{Datasource: d, Data: data, EventType: utils.CapabilitiesEventType})
					return nil
				}, opPriority)
				if err != nil {
					return err
...
func (ct *CapabilitiesTracer) callback(event *utils.EnrichEvent) {
