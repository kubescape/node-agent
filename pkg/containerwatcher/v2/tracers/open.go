...
func (ot *OpenTracer) openEventCallback(data datasource.Data) {
	...
	openEvent := &events.OpenEvent{Data: data}
...
func (ot *OpenTracer) handleEvent(event *events.OpenEvent, syscalls []uint64) {
	if ot.eventCallback != nil {
		...
		EnrichEvent(ot.thirdPartyEnricher, event, syscalls, ot.eventCallback, "", 1)
	}
}
