...
func (nt *NetworkTracer) Start(ctx context.Context) error {
	nt.gadgetCtx = gadgetcontext.New(
		...
		gadgetcontext.WithDataOperators(
			KubeIPResolverOperator,
			KubeNameResolverOperator,
	...
	go func() {
		err := nt.runtime.RunGadget(nt.gadgetCtx, nil, nil)
		if err != nil {
		...
		simple.OnInit(func(gadgetCtx operators.GadgetContext) error {
			for _, d := range gadgetCtx.GetDataSources() {
				...
				err := d.Subscribe(func(source datasource.DataSource, data datasource.Data) error {
					...
					nt.callback(&utils.EnrichEvent{Datasource: d, Data: data, EventType: utils.NetworkEventType})
					return nil
				}, opPriority)
				if err != nil {
					return err
...
func (nt *NetworkTracer) callback(event *utils.EnrichEvent) {
