...
func NewEventHandlerFactory(
	...
	dnsAdapter := NewManagerAdapter(func(eventType utils.EventType, event utils.K8sEvent) {
		...
		case utils.DnsEventType:
			if dnsEvent, ok := event.(*utils.EnrichEvent); ok {
