...
func (ch *IGHostWatcher) ptraceEventCallback(event *tracerptracetype.Event) {
	if event.Type != types.NORMAL || event.Pid == ch.ownPid {
		return
	}
...
func (ch *IGHostWatcher) startPtraceTracing() error {
...
func (ch *IGHostWatcher) stopPtraceTracing() error {
