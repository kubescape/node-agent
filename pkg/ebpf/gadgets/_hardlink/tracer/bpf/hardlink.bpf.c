...
// This gadget is used to trace the link and linkat syscalls.
SEC("tracepoint/syscalls/sys_enter_link")
int tracepoint__sys_link(struct syscall_trace_enter *ctx)
{
    struct event *event;
    u32 zero = 0;
...
int tracepoint__sys_linkat(struct syscall_trace_enter *ctx)
{
    struct event *event;
    u32 zero = 0;
    event = bpf_map_lookup_elem(&empty_event, &zero);
