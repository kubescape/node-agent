...
func (ns *NetworkStream) ReportEnrichedEvent(enrichedEvent *events.EnrichedEvent) {
	...
	case utils.NetworkEventType:
		networkEvent, ok := enrichedEvent.Event.(*utils.EnrichEvent)
		if !ok {
			return
		}
	...
	case utils.DnsEventType:
		...
		dnsEvent, ok := enrichedEvent.Event.(*utils.EnrichEvent)
		if !ok {
			return
		}
...
func (ns *NetworkStream) ReportEvent(eventType utils.EventType, event utils.K8sEvent) {
	switch eventType {
	case utils.NetworkEventType:
		networkEvent, ok := event.(*utils.EnrichEvent)
		if !ok {
			return
		}
	...
	case utils.DnsEventType:
		...
		dnsEvent, ok := event.(*utils.EnrichEvent)
		if !ok {
			return
		}
		if !ns.shouldReportDnsEvent(dnsEvent) {
...
func (ns *NetworkStream) handleDnsEvent(event *utils.EnrichEvent, processTree *apitypes.ProcessTree) {
	ns.eventsStorageMutex.Lock()
...
func (ns *NetworkStream) shouldReportDnsEvent(dnsEvent *utils.EnrichEvent) bool {
...
func (ns *NetworkStream) handleNetworkEvent(event *utils.EnrichEvent, processTree *apitypes.ProcessTree) {
...
func (ns *NetworkStream) buildNetworkEvent(event *utils.EnrichEvent, processTree *apitypes.ProcessTree) apitypes.NetworkStreamEvent {
	var domain string
	var ok bool
...
func getNetworkEndpointIdentifier(event *utils.EnrichEvent) string {
