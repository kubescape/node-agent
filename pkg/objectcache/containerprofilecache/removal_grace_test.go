...
// Projected profile must stay resolvable for a grace window after the remove callback, then be evicted — in-flight terminal events must not lose their profile (else ProfileDependency=Required rules silently suppress as profile_incomplete). Evidence: CI 31846699597 Test_48 init "setup" — 0 R0001 on terminal exec despite adopted profile.
