...
func TestContainerStrategy_GetNewParentPID(t *testing.T) {
	...
	processMap.Set(shimPID, &armotypes.Process{PID: shimPID, PPID: 1, Comm: "shim"})
	processMap.Set(containerInitPID, &armotypes.Process{PID: containerInitPID, PPID: shimPID, Comm: "init"})
	processMap.Set(parentPID, &armotypes.Process{PID: parentPID, PPID: containerInitPID, Comm: "parent"})
	processMap.Set(childPID, &armotypes.Process{PID: childPID, PPID: parentPID, Comm: "child"})
...
func TestContainerStrategy_GetNewParentPID_InitExits(t *testing.T) {
	...
	processMap.Set(shimPID, &armotypes.Process{PID: shimPID, PPID: 1, Comm: "shim"})
	processMap.Set(containerInitPID, &armotypes.Process{PID: containerInitPID, PPID: shimPID, Comm: "init"})
	processMap.Set(childPID, &armotypes.Process{PID: childPID, PPID: containerInitPID, Comm: "child"})
