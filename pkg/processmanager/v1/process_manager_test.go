	...
	testCases := []struct {
		...
		},
		{
			...
			verify: func(t *testing.T, pm *ProcessManager) {
				...
				require.True(t, exists)
				hasChild := false
				parent.ChildrenMap.Range(func(k apitypes.CommPID, v *apitypes.Process) bool {
					if v.PID == 1003 {
						hasChild = true
						return false
					}
...
func TestDuplicateProcessHandling(t *testing.T) {
	...
	t.Run("update process with same parent", func(t *testing.T) {
		...
		assert.Equal(t, 1, parent.ChildrenMap.Len())
		child, exists := parent.ChildrenMap.Load(apitypes.CommPID{Comm: "child-process", PID: 1002})
		require.True(t, exists)
		assert.Equal(t, uint32(1002), child.PID)
		...
		require.True(t, exists)
		assert.Equal(t, 1, updatedParent.ChildrenMap.Len())
		child, exists = updatedParent.ChildrenMap.Load(apitypes.CommPID{Comm: "child-process", PID: 1002})
		require.True(t, exists)
		assert.Equal(t, "child-process --updated", child.Cmdline)
	})
	...
	t.Run("update process with different parent", func(t *testing.T) {
		...
		require.True(t, exists)
		assert.Equal(t, 0, oldParent.ChildrenMap.Len())
		...
		require.True(t, exists)
		hasChild := false
		containerProcess.ChildrenMap.Range(func(k apitypes.CommPID, v *apitypes.Process) bool {
			if v.PID == 1002 {
				hasChild = true
				assert.Equal(t, "child-process --new-parent", v.Cmdline)
				return false
			}
			return true
		})
...
func TestProcessReparenting(t *testing.T) {
	...
	t.Run("reparent to nearest living ancestor", func(t *testing.T) {
		...
		require.True(t, exists)
		hasChild := false
		grandparent.ChildrenMap.Range(func(k apitypes.CommPID, v *apitypes.Process) bool {
			if v.PID == childPID {
				hasChild = true
				return false
			}
...
func TestRemoveProcessesUnderShim(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			initialTree: map[apitypes.CommPID]*apitypes.Process{
				{PID: 100}: {PID: 100, PPID: 1, Comm: "shim", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}},     // shim process
				{PID: 200}: {PID: 200, PPID: 100, Comm: "parent", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // direct child of shim
				{PID: 201}: {PID: 201, PPID: 200, Comm: "child1", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // child of parent
				{PID: 202}: {PID: 202, PPID: 200, Comm: "child2", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // another child of parent
			},
			shimPID: 100,
			expectedTree: map[apitypes.CommPID]*apitypes.Process{
				{PID: 100}: {PID: 100, PPID: 1, Comm: "shim", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // only shim remains
			},
		...
		},
		{
			...
			initialTree: map[apitypes.CommPID]*apitypes.Process{
				{PID: 100}: {PID: 100, PPID: 1, Comm: "shim", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}},     // shim process
				{PID: 200}: {PID: 200, PPID: 100, Comm: "parent", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // direct child of shim
				{PID: 201}: {PID: 201, PPID: 999, Comm: "orphan", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // orphaned process (parent doesn't exist)
			},
			shimPID: 100,
			expectedTree: map[apitypes.CommPID]*apitypes.Process{
				{PID: 100}: {PID: 100, PPID: 1, Comm: "shim", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}},     // shim remains
				{PID: 201}: {PID: 201, PPID: 999, Comm: "orphan", ChildrenMap: maps.SafeMap[apitypes.CommPID, *apitypes.Process]{}}, // orphan unaffected
			},
