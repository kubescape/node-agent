...
run_both() {
  ...
  kubectl exec -n "$NS" "$POD" -c nginx -- cat /etc/hosts >/dev/null 2>&1 || true
  kubectl exec -n "$NS" "$POD" -c nginx -- \
    curl -sm2 http://fusioncore.ai 2>/dev/null || true
  sleep 30
  ...
  kubectl exec -n "$NS" "$POD" -c nginx -- \
    curl -sm2 http://evil.example.com 2>/dev/null || true
  sleep 30
