...
const (
	dynamicIdentifier  = "⋯"
	wildcardIdentifier = "*"
	// execArgsWildcard ("⋯⋯") is the standalone zero-or-more exec-args
	// sentinel (dynamicpathdetector.ExecArgsWildcard). It is a single valid
	// token even though it contains two dynamicIdentifier runes.
	execArgsWildcard = "⋯⋯"
)
...
func LintApplicationProfile(ap *applicationProfileLike, src string) []Violation {
	...
	for ci, c := range ap.Spec.Containers {
		...
		for ei, e := range c.Execs {
			...
			// R-AP-12: args[0] must be an absolute path. The eBPF tracer
			// captures argv[0] as the full binary path; profile entries that
			// use a basename (e.g. "sh" instead of "/bin/sh") silently fail
			// to match at runtime. Caught the hard way on Test_32's first CI
			// run (PR #37 run 25178930763). args[0] need NOT equal exec.path:
			// busybox-style applets are invoked as a symlink (argv[0] =
			// /bin/sleep) whose resolved exepath differs (/bin/busybox), so
			// only absoluteness is required. Exception: args[0] may be the
			// wildcard token if the user genuinely means "any binary here".
			if !strings.HasPrefix(e.Args[0], "/") && e.Args[0] != wildcardIdentifier {
				add("R-AP-12", fmt.Sprintf("containers[%d].execs[%d].args[0] = %q must be an absolute path (eBPF captures argv[0] as full path)", ci, ei, e.Args[0]))
			}
			...
			for ai, a := range e.Args {
				...
				if strings.Contains(a, dynamicIdentifier) && a != dynamicIdentifier && a != execArgsWildcard {
