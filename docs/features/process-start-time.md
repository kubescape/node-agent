...
> hardening. During that same window the shared tree node still carries the dead
> process's `comm`, `cmdline` and `path`, so an alert can still name the wrong
> command. That is tracked separately.

The entry is deleted at the same point the tree node is deleted, so the map
cannot outgrow the tree.
