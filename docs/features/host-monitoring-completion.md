...
**Consequence for real containers.** `creator.go`'s `setProfileMetadata` used to check `if state != nil` before attaching `ProfileMetadata` — but `GetContainerProfileState` never actually returns `nil` (it synthesizes an error state when profile data is absent), so that check was dead code, and every alert got a `ProfileMetadata{Type, ProfileDependency, Error}` regardless of whether a profile actually existed. The new check, `state.Error == nil`, is genuinely presence-based: a real container alerting before its profile is cached now emits **no `ProfileMetadata` at all**, instead of one carrying an `Error` string. This is a real, if narrow, change to the alert payload for real (non-host) containers in the pre-profile window — the in-repo consumer (`pkg/exporters/alert_manager.go`) is nil-safe against a missing `ProfileMetadata`, but any external consumer that expected the `Error`/`Type`/`ProfileDependency` fields during that window will no longer see them.
